...
while current_run < number_of_runs
    ...
    if run == current_run
        ...
        try
            tmp = load(jgit(['AnalyzeData/Thresholds/glm_noremove001_max_dist_', num2str(groupsize)]));
        ...
        for iter = iter_start:niters
            ...
            if iter > 1
                tmp = load(jgit(['AnalyzeData/Thresholds/glm_noremove001_max_dist_', num2str(groupsize)]));
                max_dist_001 = tmp.max_dist_001;
                tmp = load(jgit(['AnalyzeData/Thresholds/glm_noremove01_max_dist_', num2str(groupsize)]), 'max_dist_01');
            ...
            [ ~, ~, ~, ~, ~, tstat ] = MVlm_multivar(  X, data, contrast, 1 );
            Fstat = tstat.^2;
            ...
            save(jgit(['AnalyzeData/Thresholds/glm_noremove001_max_dist_', num2str(groupsize)]), 'max_dist_001')
            save(jgit(['AnalyzeData/Thresholds/glm_noremove01_max_dist_', num2str(groupsize)]), 'max_dist_01')
        end
    end
end
